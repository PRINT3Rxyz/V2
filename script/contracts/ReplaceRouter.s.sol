...
contract ReplaceRouter is Script {
    ...
    // Array of all reward trackers
    address[] rewardTrackers = [address(0)];
    ...
    function run() public {
        vm.startBroadcast();
        // deploy new router
        Router router = new Router(marketFactory, market, priceFeed, usdc, weth, positionManager);
        ...
        // setHandler for rewardTrackers for new router and remove for old router
        for (uint256 i = 0; i < rewardTrackers.length;) {
            address rewardTracker = rewardTrackers[i];
            RewardTracker(rewardTracker).setHandler(address(router), true);
