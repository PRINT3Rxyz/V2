...
contract Deploy is Script {
    ...
    string priceUpdateSource = 'const { Buffer } = await import("node:buffer");'
        ...
        '"Content-Type": "application/json",' '"X-CMC_PRO_API_KEY": secrets.apiKey,' "}," "params: {" "symbol: tickers,"
        "time_end: timestamp," "}," "});" "if (cmcResponse.status !== 200) {"
        ...
        "const precisionBuffer = Buffer.alloc(1);" "precisionBuffer.writeUInt8(2, 0);" "const quotes = priceData.quote;"
        "let low, med, high;" "if (isLatest) {" "low = med = high = Math.round(quotes.USD.price * 100);" "} else {"
    ...
    function run() external returns (Contracts memory contracts) {
        ...
        contracts.marketFactory.setFeedValidators(
            activeNetworkConfig.contracts.chainlinkFeedRegistory,
