...
contract Deploy is Script {
    ...
    string cumulativePnlSource = 'const ethers = await import("npm:ethers@6.10.0");'
        ...
        "    BTC: 1e8," "    ETH: 1e18," "  };" "  return baseUnits[ticker] || 1e18; // Default to 1e18 if not found"
        ...
        "    timestamp: Math.floor(Date.now() / 1000), // Ensure correct timestamp" "    cumulativePnl: cumulativePnl,"
        "  };" "};" "" "const formatResult = (result) => {" "  const buffer = Buffer.alloc(23);" ""
        "  // Precision (1 byte)" "  buffer.writeUInt8(result.precision, 0);" "" "  // Timestamp (6 bytes)"
